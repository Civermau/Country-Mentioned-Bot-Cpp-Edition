...
int main() {
  ...
  while (
      BOT_TOKEN.empty()) { // * Stuff to do when there's no token, not important
    char buffer[100];
    std::cout << "BOT_TOKEN is empty, please provide a token" << std::endl;
    std::cout << "If you write wrong the token the program won't close (I'll "
                 "fix this later), so be careful"
              << std::endl;
    std::cout << "Token will be automatically saved to the .env file"
              << std::endl;
    std::cout << "To get a token, you can follow this guide: "
                 "https://discord.com/developers/docs/quick-start/"
                 "getting-started#step-1-creating-an-app"
              << std::endl;
    std::cout << "Token: ";
  ...
  std::cout << "[TODO] Find a way to load contries dinamically instead of "
               "hardcoded (If I want to add more languages)"
            << std::endl
            << std::endl;
  ...
  bot.on_message_create([&bot, &countries](const dpp::message_create_t &event) {
    ...
    std::cout << "NEW MESSAGE: " << uppercaseMessage
              << std::endl; // FUCK YOU BUFFER
  ...
  bot.on_ready([&bot](const dpp::ready_t &event) {
    // bot.global_bulk_command_delete();  // Uncomment this if somehow you
    // messed up and added unwanted commnads, THIS WILL DELETE EVERY COMMAND

    if (dpp::run_once<struct register_bot_commands>()) {
      // Register slash commands
      bot.global_command_create(
          dpp::slashcommand("info", "Get info about the bot", bot.me.id));
      std::cout << std::endl << "[DEBUG]  Commands registered" << std::endl;
    }
  });
...
dpp::embed buildInfoEmbed(std::vector<TopCountryData> topCountries) {
  dpp::embed infoEmbed =
      dpp::embed()
          ...
          .set_description(
              "Let Yui inform you that you've just mentioned a country!")
  ...
  if (topCountries.size() >= 1) {
    infoEmbed.add_field("Most mentioned country:",
                        "**" + topCountries[0].name + "** with " +
                            topCountries[0].mentions + " mentions!");
  }

  if (topCountries.size() >= 2) {
    infoEmbed.add_field("Second place:",
                        "**" + topCountries[1].name + "** (" +
                            topCountries[1].mentions + ")",
                        true);
  }
...
std::string toUpperUTF8(const std::string &str) {
  ...
  std::locale loc("");  // or "" for system locale
  for (auto& c : wide) {
    c = std::toupper(c, loc);      // locale-aware toupper
  }
